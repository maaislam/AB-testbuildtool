...
const productCard = (id, data, index) => {
  ...
  const htmlStr = `<div class="${id}__productCard product-card ${
    ...
    data.url
        ...
        <a href="${data.productLink}"><img class="${id}__productImage" src="${
    data.imgSrc
  }" alt="Roll Image" /></a>
        <div class="${id}__productContent">
            <a href="${data.productLink}" class="product-title">${data.productTitleText}</a>
    ...
    data.price
            ...
            <div class="${id}__addedToBasket ${id}__hide">Added to cart</div>
        </div>
    </div>`;
