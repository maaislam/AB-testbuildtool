...
const init = async (mutation) => {
  ...
  setTimeout(() => {
    ...
    document.querySelector('.Cart ').submit((e) => {
      // alert("Submitted");
      if (goalFired == false) {
        e.preventDefault();
        setTimeout(() => {
  ...
  const productArray = [];
  document.querySelectorAll('.CartItem__Info .QuantitySelector').forEach((item, key) => {
    productArray.push(item.querySelector('input').getAttribute('value'));
    item.querySelector('input').addEventListener('change', () => {
      if (parseInt(item.querySelector('input').value) > parseInt(productArray[key])) {
        const prodHeading = item
  ...
  if (document.querySelector('body').classList.contains(`${ID}_plus-clicked`)) {
    const ind = document.querySelector('body').getAttribute('id');
    const productHeading = items[ind].product_title.split('|')[0];
    const prodQty = items[ind].quantity;
  ...
  if (isCartPage) {
    ...
    increaseQtyBtn.forEach((item, key) => {
