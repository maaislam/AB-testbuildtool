const combo = [
  ...
  {
    name: 'Gunn Oxford Sports Fit Shirt',
    image: 'https://www.roddandgunn.com/dw/image/v2/BBRS_PRD/on/demandware.static/-/Sites-roddandgunn-master-catalog/default/dw0839a33b/images/006582/LP5261_TARMAC_FT_LGE.jpg?sw=166&sh=205&sm=fit',
    vgid: '006582-04',
    color: 'TARMAC'
  },
  {
  ...
  {
    name: 'Ls Gunn Polo',
    image: 'https://www.roddandgunn.com/dw/image/v2/BBRS_PRD/on/demandware.static/-/Sites-roddandgunn-master-catalog/default/dw368061b8/images/004539/KP0008_ICE_FT_LGE.jpg?sw=166&sh=205&sm=fit',
    vgid: '004539-39',
    color: 'ICE'
  },
  {
  ...
  },
  {
    ...
    vgid: '007754-01',
    image: 'https://www.roddandgunn.com/dw/image/v2/BBRS_PRD/on/demandware.static/-/Sites-roddandgunn-master-catalog/default/dw4f9413f2/images/007754/ZX0496_NERO_FT_LGE.jpg?sw=166&sh=205&sm=fit',
    color: 'NERO'
  },
  {
    name: 'The Cascades Sports Fit Jacket',
    vgid: '007345-04',
    image: 'https://www.roddandgunn.com/dw/image/v2/BBRS_PRD/on/demandware.static/-/Sites-roddandgunn-master-catalog/default/dwd1b27d37/images/007345/BP1322_SAND_FT_LGE.jpg?sw=166&sh=205&sm=fit'
    //color: 'ADRIATIC'
  },
  {
...
const isIncludesFromProdList = () => {
  const combineProds = combo;
  //console.log('combine', combineProds);
  const cartProduct = document.querySelectorAll(
    '#cart-table > tbody > tr > td.item-details > div.product-list-item > div.item-name-container > div > a'
  );

  const formattedCartProds = Array.from(cartProduct);

  const filteredCombineProducts = [];
  while (filteredCombineProducts.length < 3) {
    const r = Math.floor(Math.random() * combineProds.length - 1) + 1;
    const isIncluded = formattedCartProds.every((name) => {
      return !(name?.innerText?.toLowerCase().includes(combineProds[r]?.name?.toLowerCase()));
    });
    //console.log(combineProds[r]?.name);
    if (filteredCombineProducts.indexOf(r) === -1 && isIncluded) {
      //console.log(combineProds[r]);
      filteredCombineProducts.push(combineProds[r]);
    }
  }
  return filteredCombineProducts;
};
...
const getTranslateX = () => {
  const style = window.getComputedStyle(
    document.querySelector(
      '#cart-items-form > fieldset > div > div.rg-8-3-similar-container > .prod-wrapper'
    )
  );
...
const setSminiliarProducts = (similiarComponents, prodLists) => {
  ...
  if (prodLists) {
    if (computedInnerWidth > 767) {
      document
        .querySelector('.rg-8-3-similar-container.sm-screen-bp')
        ?.classList.add('rg-8-3-none');
      document
        .querySelector('.rg-8-3-similar-container.lg-screen-bp')
        ?.classList.remove('rg-8-3-none');

      //console.log('> 767');
      if (!document.querySelector('.rg-8-3-similar-container.lg-screen-bp')) {
        document
      ...
      //console.log('<= 767');
      if (!document.querySelector('.rg-8-3-similar-container.sm-screen-bp')) {
        document
      ...
      item.querySelector(`#${ID}-quick-cart`).addEventListener('click', async () => {
        ...
        try {
          const response = await fetch(
            `https://www.roddandgunn.com/us/Product-Variation?pid=${prodLists[index].vgid}&dwvar_${prodLists[index].vgid}_color=${prodLists[index].color}&source=tile&attribute=color`
          );
          ...
          if (data) {
            ...
            Array.from(item.querySelectorAll('.selectable a')).forEach((li) => {
              ...
              li.addEventListener('click', () => {
                addToCart(prodLists[index].vgid, li.innerText, prodLists[index].name);
              });
            });
          }
    ...
    const indecatores = Array.from(
      document.querySelectorAll(
        '#cart-items-form .rg-8-3-similar-container .product-indecator span'
      )
    );
...
const WidthChange = () => {
  ...
  const similiarComponents = prodLists.map((item) => {
    return `<div class="similar-product">
                <img src="${item.image}" alt="${item.name}">
                <p class="product-name">${item.name}</p>
                <button type="button" id="${ID}-quick-cart" class="similar-product--cart">+ ADD TO SHOPPING BAG</button>
...
waitForEl(
  ...
  () => {
    ...
    try {
      ...
      //waitForEl('#cart-table > tbody > tr:last-child > td.item-details > div.product-list-item > div.item-name-container > div > a', () => {
      //const cartProd = Array.from(document.querySelectorAll('#cart-table > tbody > tr > td.item-details > div.product-list-item > div.item-name-container > div.name > a'));
      //const getProductUrl = () => {
      //return cartProd.map((item) => item.href);
      //};
