...
const collectProductInfo = (
  ...
  const textTimeElement = doc.querySelector('.MuiTypography-body1.mui-1l31lym');
  const textTime = textTimeElement?.textContent.trim() || '';
  ...
  for (const [index, response] of responses.entries()) {
    if (response) {
      ...
      collectProductInfo(
        ...
        urls[index].firstTeam,
        urls[index].secondTeam
      );
    }
  }
